...
class SitesClient(ResourceClient):
    ...
    def update_domain(self, new_domain: str, site_id: Optional[int] = None, domain: Optional[str] = None, keep_old_domain: bool = False) -> Job | None:
        """
    ...
    def restore_site(self, restore_from_fs: int, restore_from_db: int, site_id: Optional[int] = None, domain: Optional[str] = None) -> Dict[str, Any]:
        ...
        Note:
            ``allow_restore`` metadata key must be set to a recent
            unix timestamp and the site must be suspended with a
        ...
        payload = [("restore_from[]", str(restore_from_fs)), ("restore_from[]", str(restore_from_db))]
        return self._post(endpoint, data=payload)
