...
def main():
    """Demonstrates querying database performance metrics."""
    ...
    print(f"\n--- Querying MySQL Performance Metrics for site '{SITE_DOMAIN}' (Last 6 hours, ending 30 mins ago) ---")
    try:
        # --- Example 1: Concurrent Connections and Slow Queries ---
        connections_metrics = client.metrics.query(
            start=start_time, end=end_time, query_type="site", key=SITE_DOMAIN,
            metric=["mysql_concurrent_connections", "mysql_max_statement_time_exceeded"],
            dimension="server"
        )
        print_metric_results("Concurrent DB Connections and Slow Queries", connections_metrics)

        # --- Example 2: Read/Write Operations ---
        rows_metrics = client.metrics.query(
            start=start_time, end=end_time, query_type="site", key=SITE_DOMAIN,
            metric=["mysql_rows_read", "mysql_rows_inserted", "mysql_rows_updated"],
            dimension="server"
        )
        print_metric_results("DB Rows Read/Written", rows_metrics)
