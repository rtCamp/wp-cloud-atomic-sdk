...
# 'wpscan' runs the WPScan vulnerability scanner; 'pnt-versions' inventories
# plugin/theme versions across sites.
SCAN_TYPE: str = "wpscan"

def main() -> None:
    """
    Demonstrates creating and monitoring a bulk 'wpcloud-scan' task using
    the dedicated `tasks.create_wpcloud_scan()` method.
    """
    ...
    try:
        print("\n--- [1/2] Creating a bulk WPCloud scan task ---")
        print(f"  - Scan type: '{SCAN_TYPE}' on all sites for client '{CLIENT_ID}'.")

        task_request: TaskCreationResponse = client.tasks.create_wpcloud_scan(
            scan_type=SCAN_TYPE,
        ...
        while True:
            ...
            print(f"  - Progress: {meta.get('success_count', 0)} sites scanned, {meta.get('failure_count', 0)} failed.")
            ...
            if task_status.complete:
                ...
                print("  - Note: Per-site scan results are delivered to your configured webhook URL.")
                break
